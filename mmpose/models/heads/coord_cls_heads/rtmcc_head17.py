...
class RTMCCHead17(BaseHead):
    ...
    def __init__(
        ...
        self.aux_cls_x = nn.Linear(gau_cfg['hidden_dims'], W, bias=False)
        self.aux_cls_y = nn.Linear(gau_cfg['hidden_dims'], H, bias=False)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        if self.training:
            aux_pred_x = self.aux_cls_x(feats)
            aux_pred_y = self.aux_cls_y(feats)
        ...
        if self.training:
            return pred_x, pred_y, aux_pred_x, aux_pred_y
    ...
    def loss(
        ...
        pred_x, pred_y, aux_pred_x, aux_pred_y = self.forward(feats)
        ...
        aux_simcc = (aux_pred_x, aux_pred_y)
        gt_simcc = (gt_x, gt_y)

        # calculate losses
        losses = dict()
        loss = self.loss_module(pred_simcc, gt_simcc, keypoint_weights)
        loss = loss + self.loss_module(aux_simcc, gt_simcc, keypoint_weights)
        loss = loss * 0.5
