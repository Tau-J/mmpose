...
class RTMCCHead13(BaseHead):
    ...
    def __init__(
        ...
        self.mlp = [
            nn.Sequential(
                ...
                nn.Linear(
                    flatten_dims * 4**i, gau_cfg['hidden_dims'], bias=False))
            for i in [2, 1, 0]
        ]
        ...
        self.gamma = nn.Parameter(torch.rand((1, 1, 2, hidden_dims)))
        self.beta = nn.Parameter(torch.rand((2, hidden_dims)))
        self.o = nn.Linear(hidden_dims, hidden_dims, bias=False)
        self.sqrt_s = math.sqrt(hidden_dims)
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        base = base.unsqueeze(2) * self.gamma + self.beta
