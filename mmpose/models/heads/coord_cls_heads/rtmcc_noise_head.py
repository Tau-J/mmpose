...
class RTMCCNoiseHead(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        if self.training:
            k = int(feats.size(1) // 5 + 1)
            mask = random.choices(list(range(feats.size(1))), k=k)
            # groups = []
            half_b = feats.size(0) // 2
            feats[:half_b, mask, :] = 0.
            # feats[:half_b, mask, :] += torch.randn(
            # half_b, 6, feats.size(2), device=feats.device, dtype=feats.dtype)
            # feats[:half_b, mask, :] += torch.rand(
            # half_b, 6, feats.size(2), device=feats.device, dtype=feats.dtype)
