...
class RTMCCHead14(BaseHead):
    ...
    def __init__(
        ...
        self.rdp_loss = MODELS.build(
            dict(type='KLDiscretLoss', use_target_weight=False))
    ...
    def loss(
        ...
        r_pred_x, r_pred_y = self.forward(feats)
        ...
        # calculate losses
        losses = dict()
        loss1 = self.loss_module(pred_simcc, gt_simcc, keypoint_weights)
        loss2 = self.loss_module(r_pred, gt_simcc, keypoint_weights)
        loss = loss1 + loss2
        r_loss1 = self.rdp_loss(pred_simcc,
                                (r_pred[0].detach(), r_pred[1].detach()),
                                keypoint_weights)
        r_loss2 = self.rdp_loss(r_pred, (pred_simcc[0].detach(),
                                         pred_simcc[1].detach()),
                                keypoint_weights)
        r_loss = (r_loss1 + r_loss2) / pred_x.size(0)
        losses.update(loss_kpt=loss, r_loss=r_loss)
