...
class RTMCCBlock2(nn.Module):
    ...
    def __init__(self,
        ...
        super(RTMCCBlock2, self).__init__()
        self.s = s
        self.num_token = num_token
        self.use_rel_bias = use_rel_bias
        self.attn_type = attn_type
        ...
        self.scale = nn.Parameter(torch.zeros(size=(1, 1, self.s)))
        self.dwc = nn.Conv1d(
            in_channels=self.e,
            out_channels=self.e,
            kernel_size=5,
    ...
    def _forward(self, inputs):
        ...
        q = self.act_fn(q) + 1e-6
        k = self.act_fn(k) + 1e-6

        scale = self.soft_plus(self.scale)
        q = q / scale
        k = k / scale
        ...
        # qk:[B, K, s] x [B, s, K] -> [B, K, K]
        qk = torch.bmm(q, k.permute(0, 2, 1))

        # [B, K, K] x [B, K, e], [B, K] -> [B, K, e]
        x = torch.einsum('b i j, b j d, b i -> b i d', qk, v, z)

        # [B, K, e] -> [B, e, K]
        feature_map = v.permute(0, 2, 1)
        feature_map = self.dwc(feature_map).permute(0, 2, 1)
        x = x + feature_map
        x = x * u
        # [B, K, e] -> [B, K, out_token_dims]
        x = self.o(x)
