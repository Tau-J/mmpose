...
import mmpose.registry as mmpose_registry_module
from mmpose.utils import register_all_modules as mmpose_register_all_modules

logger = MMLogger('convert', log_file='./debug.log')
...
def collect_all_modules():
    mmpose_registries = [
        value for value in mmpose_registry_module.__dict__.values()
    ...
    all_registries = set(mmpose_registries + mmengine_registries)
...
def convert_files(args):
    ...
    for filepath in track(all_files):
        ...
        # We could not do the conversion and validation at the same time
    ...
    for file, target_file in track(list(zip(all_files, target_files))):
        try:
            ...
            print(f'failed to validate file {file} and {target_file} for {e}'
                  f'see error information in ./debug.log')
            # raise e
    # print(target_files)
    # print('done')
